...
DM = function(async, loader, config) {
    ...
    //serviceDeferredProvider   = new ServiceProvider(this, async, { deferred: true });
    parameterProvider = new ParameterProvider(this, async);
    resourceProvider  = new ResourceProvider(this, async);
    //resourceDeferredProvider  = new ResourceProvider(this, async, { deferred: true });
...
DM.prototype = {
    ...
    get: function(key, options) {
        var self = this,
            definition, promise,
            alias,
            isShared, isSynthetic, isAlias, isSingleProperty, isLazy, isDeferred,
            forthcoming;
        ...
        if (isSynthetic && !this.initialized(key)) {
            ...
            if (isDeferred) {
                return this.async.resolve(function() {
        ...
        if (isDeferred && !isLazy) {
            return this.async.resolve(function() {
                var promise;

                promise = self.build(definition);

                if (isShared) {
                    self.services[key] = promise;
                }
        ...
        if (isLazy) {
            promise = this.async.resolve(function() {
    ...
    build: function(definition, slugs) {
        ...
        return this.loadParser.parse(definition.path)
            .then(function(operand) {
                return self.parse(definition, parser)
                    .then(function(def) {
